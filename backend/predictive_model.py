...
class AdvancedNBAPlayerPredictor:
    ...
    def create_features(self, all_games, opponent_abbr, season_type, category_type='offensive'):
        ...
        # Normalise opponent strength against the current season's league average.
        # A player may have no games in the current season yet (injury, rookie,
        # early-season), in which case `league_avgs` is keyed only by the seasons
        # actually present in their log -- fall back to the most recent of those
        # rather than raising a KeyError.
        baseline_season = bc.CURRENT_SEASON
        if baseline_season not in league_avgs:
            baseline_season = max(league_avgs) if league_avgs else None
        for stat in self.stat_categories:
            baseline = league_avgs[baseline_season][f'opp_{stat}'] if baseline_season else 0
            if not baseline:
                logger.warning("No league baseline for %s; leaving opp_strength neutral", stat)
                all_games[f'opp_strength_{stat}'] = 1.0
            else:
                all_games[f'opp_strength_{stat}'] = all_games[f'opp_{stat}'] / baseline
        ...
        advanced_stats = bc.get_player_advanced_stats(player_id, bc.CURRENT_SEASON, season_type)
        usg_pct = advanced_stats.get('USG_PCT', 0.2)
        ts_pct = advanced_stats.get('TS_PCT', 0.5)
        fatigue_metrics = bc.get_player_fatigue_metrics(player_id, bc.CURRENT_SEASON, season_type, num_games=10)
    ...
    def predict_over_under(self, player_id, category, opponent_abbr, season_type, betting_line, category_type='offensive', seasons=None):
        ...
        averages = bc.get_player_season_recent_averages(player_id, bc.CURRENT_SEASON, season_type)
        ...
        opp_recent_stats = bc.get_team_recent_stats(opp_team_id, bc.CURRENT_SEASON, season_type, measure_type, num_games=10)
        opp_avgs = {
